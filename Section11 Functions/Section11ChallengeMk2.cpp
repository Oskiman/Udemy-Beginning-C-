...
// Section Challenge Mk2
// Modularise Section 9 challenge to use functions
/*
 * I have moved the code from the menu_choice() function into main and
 * dispensed with menu_choice()
 * in order to preserve const correctness.  The numbers_input vector no longer
 * needs to be passed into & then on from menu_choice().  It needed to
 * be passed non-const for the add_number() function to be able
 * to change the elements within.  This, of course left numbers_input open
 * to possible, unintended change within menu_choice and possibly
...
char get_user_input();
void sum_list(const vector <int> &numbers_input);
void product_list(const vector <int> &numbers_input);
    ...
    do{
        ...
        switch(user_selection){
            ...
            case 'C':
                ...
                break;
            case 'U':
                sum_list(numbers_input);
                break;
            case 'O':
                product_list(numbers_input);
                break;
            case 'Q':
                cout << "Goodbye!" << endl;
                break;
...
void display_menu(){
    ...
    cout << "U - Sum the contents of the list" << endl;
    cout << "O - Product of the numbers of the list" << endl;
    cout << "Z - Sort list in order, smallest first" << endl;
    cout << "Q - Quit" << endl;
...
void product_list(const vector <int> &numbers_input){
    int prod_total{1};      // Initialised to 1
    for(auto num : numbers_input){
        prod_total *= num;
    }
    
    cout << prod_total << endl;
}
